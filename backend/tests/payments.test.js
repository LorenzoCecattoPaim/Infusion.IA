...
test("createPayment gera order_nsu interno e usa userId do backend", async () => {
  const store = {
    payment_orders: [],
  };
  ...
  assert.equal(store.payment_orders[0].gateway_order_id, `gw-${result.orderId}`);
});

test("processWebhook aprova pedido, atualiza banco e impede duplicação de créditos", async () => {
  const store = {
    payment_orders: [
      {
        id: "test123",
...
test("processWebhook não quebra com pedido inexistente", async () => {
  const supabase = createFakeSupabase({
...
test("webhook responde 200 rápido e processa em background", async () => {
